...
def format_insights_as_story(
    ...
    if not use_llm or (llm_provider == 'gemini' and not os.environ.get('GOOGLE_API_KEY')) or \
       (llm_provider == 'openai' and not os.environ.get('OPENAI_API_KEY')):
        return generate_basic_story(df, insights)
    
    try:
        # Try to generate story with LLM
        if llm_provider == 'gemini':
            from .llm_insights_gemini import generate_story_with_llm
        else:
            from .llm_insights_openai import generate_story_with_llm
...
def format_insights_as_summary(
    ...
    try:
        ...
        if llm_provider == 'gemini':
            ...
            from .llm_insights_openai import generate_summary_with_llm
